...
import setup_log_dir as _mod  # noqa: E402
...
def test_resolve_log_path_project_name_replaced() -> None:
    ...
    assert resolve_log_path_from_env("/var/log/{PROJECT_NAME}", "myapp", "Linux") == "/var/log/myapp"
    assert (
        resolve_log_path_from_env("/var/log/{PROJECT_NAME}/sub", "py-template", "Linux") == "/var/log/py-template/sub"
    )
...
def test_resolve_log_path_home_expanded_on_linux() -> None:
    ...
    try:
        assert resolve_log_path_from_env("$HOME/logs/myapp", "myapp", "Linux") == "/home/user/logs/myapp"
    finally:
...
def test_resolve_log_path_home_expanded_on_darwin() -> None:
    ...
    try:
        assert (
            resolve_log_path_from_env("$HOME/Library/Logs/{PROJECT_NAME}", "myapp", "Darwin")
...
def test_resolve_log_path_windows_userprofile() -> None:
    ...
    try:
        result = resolve_log_path_from_env("%USERPROFILE%\\AppData\\Local\\{PROJECT_NAME}", "myapp", "Windows")
